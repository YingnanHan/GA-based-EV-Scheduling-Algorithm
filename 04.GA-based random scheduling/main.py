...
class GeneticAlgorithm: # 定义遗传算法类
    def __init__(self,t_c_l,ev_ct,ev_cp,n,c_rate=0.7,m_rate=0.3,pop_size=200,maxnum=200): # 初始化遗传算法执行下来所需要的所有变量
    ...
    def cross(self,parent1,parent2): # 两个个体的交叉操作 产生一个新的个体
        ...
        index2 = np.random.randint(index1,len(parent1)) # 随机找到一个交叉的结束点
        ...
        newGene = [] # 用于存储新的基因
        p1len = 0
        # print("before1",parent1)
        # print("before2",parent2)
    ...
    def solve(self): # EV问题的解决方案
        ...
        print("Iteration:%d, fitness:%f, average fitness:%f, best individual:%s.\n" % (self.t, self.trace[self.t, 0], self.trace[self.t, 1], str(self.bestgene.fitness))) # 输出当前适应度的信息
        ...
        print("Average of queueing time (minute):",self.besttruefitness[3]/899)
        ...
        print("Total revenues of EVCS:",self.besttruefitness[0])
